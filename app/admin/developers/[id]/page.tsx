...
export default async function AdminDeveloperProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [posts, communityListings] = await Promise.all([
    listSellerCommunityPosts(developer.id, user.id),
    listSellerCommunityListings(developer.id, user.id)
  ]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Link href="/admin/developers" className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-800">
      ...
      <section className="space-y-3">
        ...
        <CommunityFeed
          ...
          postsRefreshUrl={`/api/community/users/${encodeURIComponent(developer.id)}/posts`}
        />
      </section>
    </div>
  );
