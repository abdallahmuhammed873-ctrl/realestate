...
import { addDeveloperProfile, getUserById, listDeveloperProfilesForAdmin } from "@/lib/repository";
...
export async function GET() {
  ...
  const items = (await listDeveloperProfilesForAdmin())
    .map(({ developer, stats }) => {
      ...
      return { developer: safeDeveloper, stats };
    })
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ ok: true, developer: toAdminDirectoryUser(created) });
}
