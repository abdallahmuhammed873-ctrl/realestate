...
function ListingStatusBadge({ status }: { status: "DRAFT" | "PENDING" | "APPROVED" | "REJECTED" }) {
  ...
  if (status === "REJECTED") return <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">Rejected</span>;
              ...
              return (
                <li key={listing.id} className="rounded-xl border p-3">
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    Created by: {seller?.name ?? "Seller"}
                    {seller?.companyOwnerId ? " (Company User)" : ""}
                  </p>
                </li>
              );
            }
