...
export async function PendingListings({ items }: { items: PendingItem[] }) {
  const language = await getRequestLanguage();
  if (items.length === 0) {
    return <p className="surface-card rounded-2xl p-6">{t(language, "noPendingListings")}</p>;
  }
