...
export async function RejectedListings({ items }: { items: RejectedItem[] }) {
  const language = await getRequestLanguage();
  if (items.length === 0) {
    return <p className="surface-card rounded-2xl p-6">{t(language, "noRejectedListings")}</p>;
  }
            ...
            <div className="grid gap-3 md:grid-cols-[220px,1fr]">
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="status-danger rounded-full px-2 py-1 text-xs font-semibold">{t(language, "rejectedStatus")}</span>
